...
public interface IKnowledgeAssetService
{
    ...
    Task<IReadOnlyList<FavoriteDocumentDto>> GetFavoritesAsync(Guid userId,CancellationToken ct);
    Task TrackRecentAsync(Guid userId,Guid documentId,CancellationToken ct);
    Task<IReadOnlyList<RecentDocumentDto>> GetRecentAsync(Guid userId,int take,CancellationToken ct);

...
public sealed class ShareService(KnowledgeDbContext db) : IShareService
{
    ...
    public async Task<ShareLinkDto?> CreateAsync(Guid documentId,Guid? userId,DateTimeOffset? expiresAt,string? password,CancellationToken ct)
    {
    ...
    public async Task<IReadOnlyList<ShareLinkDto>> GetDocumentLinksAsync(Guid documentId,CancellationToken ct)
        => await db.DocumentShareLinks.AsNoTracking()
            .Where(x=>x.DocumentId==documentId)
            .OrderByDescending(x=>x.CreatedAt)
            .Select(x=>new ShareLinkDto(x.Id,x.DocumentId,x.Token,x.ExpiresAt,x.Enabled,x.PasswordHash!=null,x.AccessCount,x.LastAccessAt,x.CreatedAt))
            .ToListAsync(ct);
    ...
    public Task<Guid?> GetDocumentIdByLinkAsync(Guid id,CancellationToken ct)
        => db.DocumentShareLinks.AsNoTracking().Where(x=>x.Id==id).Select(x=>(Guid?)x.DocumentId).FirstOrDefaultAsync(ct);

    public async Task<SharedDocumentDto?> GetSharedDocumentAsync(string token,string? password,string? ipAddress,string? userAgent,CancellationToken ct)
    {
        ...
        var document=await(from d in db.Documents.AsNoTracking()
            ...
            select new SharedDocumentDto(d.Id,d.Title,c.Markdown,link.ExpiresAt)).FirstOrDefaultAsync(ct);
    ...
    public async Task<IReadOnlyList<ShareAccessLogDto>> GetAccessLogsAsync(Guid documentId,int take,CancellationToken ct)
        => await(from log in db.ShareAccessLogs.AsNoTracking()
            join link in db.DocumentShareLinks.AsNoTracking() on log.ShareLinkId equals link.Id
            where link.DocumentId==documentId
            orderby log.AccessedAt descending
            select new ShareAccessLogDto(log.Id,log.ShareLinkId,log.IpAddress,log.UserAgent,log.Success,log.Message,log.AccessedAt))
            .Take(Math.Clamp(take,1,500)).ToListAsync(ct);

    private async Task WriteLogAsync(Guid linkId,string? ip,string? userAgent,bool success,string message,CancellationToken ct)
    {

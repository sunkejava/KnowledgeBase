...
onMounted(async () => {
  try {
    doc.value = (await shareApi.publicDocument(String(route.params.token))).data
  } catch {
    notFound.value = true
  } finally {
    loading.value = false
